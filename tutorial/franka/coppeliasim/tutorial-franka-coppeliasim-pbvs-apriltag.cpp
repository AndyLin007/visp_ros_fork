...
main( int argc, char **argv )
{
    ...
    for ( int i = 1; i < argc; i++ )
    {
        ...
        else if (std::string(argv[i]) == "--upperright")
        {
            corner = "upperright";
        }
        else if (std::string(argv[i]) == "--upperleft")
        {
            corner = "upperleft";
        }
        else if (std::string(argv[i]) == "--lowerright")
        {
            corner = "lowerright";
        }
        else if (std::string(argv[i]) == "--lowerleft")
        {
            corner = "lowerleft";
        }
    ...
    try
    {
        ...
        // Being on top of left tote
        vpHomogeneousMatrix fM_eed_l_tote(vpTranslationVector(0.45, 0.4, 0.15),
                                          vpRotationMatrix( {1, 0, 0, 0, -1, 0, 0, 0, -1} ) );
        // Being on top of right tote
        vpHomogeneousMatrix fM_eed_r_tote(vpTranslationVector(0.45, -0.4, 0.15),
                                          vpRotationMatrix( {1, 0, 0, 0, -1, 0, 0, 0, -1} ) );
...
//      				  obj_vec[j].wMo = robot.get_fMe()*eMc*cMo_vec[idx]; // if you assume the objects are static, you can comment this line
            ...
            if( State == 0) {
                ...
                active_cdMc = (fM_l_tote* l_toteM_tag*edMo.inverse()*eMc).inverse()*robot.get_fMe()*eMc ;
                t.buildFrom(active_cdMc);
                tu.buildFrom(active_cdMc);
                if ( !servo_started )
                {
            ...
            if(error_tr <= 0.02 && error_tu <= 5 && State == 0)
            {
                std::cout << "Go to the right tote \n";
            ...
            else if(error_tr <= 0.02 && error_tu <= 5 && State == 2){ // once reached the position above the right tote, move to the left tote

                std::cout << "Moving the parcel to the left tote \n";
                State = 3;
                servo_started = false;
            }
            else if(error_tr <= 0.02 && error_tu <= 5 && State == 3){ // once reached the position above the left tote, place the box
